...
Jarvis теперь регистрирует конкретные сочетания через Windows `RegisterHotKey`.
Поведение нового provider без elevation пока не проверено, поэтому до ручной
проверки v1.2.6 запускайте терминал от администратора.
...
- Поведение нативного provider без прав администратора пока не проверено.
  До ручной проверки v1.2.6 Jarvis сохраняет предупреждение при запуске и
  рекомендацию запускать процесс elevated. Это временное неподтверждённое
  ограничение, а не установленное требование `RegisterHotKey`.
