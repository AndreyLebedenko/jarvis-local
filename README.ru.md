...
- В live-режиме Status Console пока нет видимого Shutdown control.
  Используйте `Ctrl+Alt+Q`, чтобы корректно остановить Jarvis; теперь этот
  hotkey работает в этом режиме и идет через штатный shutdown path. `Ctrl+C` из
  терминала по-прежнему не является надёжным способом остановки, пока foreground
  UI loop принадлежит `pywebview`.
