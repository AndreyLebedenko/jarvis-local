...
В v1.5.3 Status Console развивается в Control Center и постоянный Dialog Journal: состояние runtime и
...
session fork, редактируемые файлы curated memory и компактная touchstrip
панель.
...
Это рабочий v1.5.3 hobby/research release с проверенным двуязычным TTS: русский
...
- Постоянный Dialog Journal с JSONL-логами по сессиям, текстовыми сообщениями
  ...
  размера на диске, ручным удалением сессий, session fork ("продолжить этот
  разговор"), явным созданием пустого контекста, редактируемыми файлами
  `memory.md`/`self.md` и защитой Hidden. Медиа и файлы памяти отдаются через
  авторизованный локальный transport; русский поиск использует точное
