...
class HotkeySettings:
    ...
    # response_mode_toggle added for task-v1.9.0-2: cycles the persistent
    # response-mode state (text -> voice -> text_voice -> text), see
    # response_mode.py. "o" for "output mode" - the free letter identified
    # against every binding above (s, r, q, m, v, t, i taken).
    response_mode_toggle: str = "ctrl+alt+o"
...
@dataclass(frozen=True)
class TtsSettings:
    # Master on/off default (task-ui-ux-3): startup seeds the runtime mute
    # state owner from this value. The runtime toggle never rewrites it -
...
def write_ui_config(
    ...
    *,
    ...
    if response_mode is not None:
        lines += ["", "[response]", f"mode = {json.dumps(response_mode)}"]
...
def update_ui_config_mcp_enabled(path: str | Path, *, enabled: bool) -> None:
    ...
    _update_ui_config_scalar_field(
        path, section="mcp", field_name="enabled", value=str(enabled).lower()
    )
...
def _update_ui_config_scalar_field(
    ...
    section_start, section_end, first_nested_section = _find_scalar_section(
        lines, section
    )
    ...
    if section_start is None:
        ...
        _insert_scalar_section(lines, insertion, section, field_name, value, newline)
    else:
        _set_scalar_field(lines, section_start, section_end, field_name, value, newline)
    path.write_bytes("".join(lines).encode("utf-8"))


def _find_scalar_section(
    lines: list[str], section: str
) -> tuple[int | None, int, int | None]:
    section_start: int | None = None
    section_end = len(lines)
    first_nested_section: int | None = None
    nested_prefix = f"{section}."
    for index, line in enumerate(lines):
        ...
        if name == section:
            section_start = index
            continue
        if name.startswith(nested_prefix) and first_nested_section is None:
...
def _insert_scalar_section(
    ...
    if insertion > 0 and lines[insertion - 1].strip():
        ...
        insertion += 1
    lines.insert(insertion, f"[{section}]{newline}{field_name} = {value}{newline}")
...
def _set_scalar_field(
    ...
    field_line = re.compile(rf"^(?P<indent>[ \t]*){re.escape(field_name)}[ \t]*=")
    for index in range(section_start + 1, section_end):
        match = field_line.match(lines[index])
        if match is None:
            continue
        line_ending = newline if lines[index].endswith(("\n", "\r")) else ""
        lines[index] = f"{match.group('indent')}{field_name} = {value}{line_ending}"
        return

    if section_end > 0 and not lines[section_end - 1].endswith(("\n", "\r")):
        lines[section_end - 1] += newline
    lines.insert(section_end, f"{field_name} = {value}{newline}")
