...
Usage:
  python day0_checks.py fidelity   nonsense.wav
  python day0_checks.py intonation neutral.wav emotional.wav
  python day0_checks.py ocr        screenshot.png
  python day0_checks.py vram
  python day0_checks.py <check> <files...> --model gemma4:12b-it-qat
...
def chat(model: str, content: str, media_b64: list[str] | None = None,
         verbose: bool = False, **options) -> str:
    ...
    if r.status_code != 200:
        ...
        print("Request (media truncated):",
              json.dumps({**payload, "messages": [{**msg, "images": ["<b64>"]}]},
    ...
    print(f"[{dt:.1f}s total | load {load:.1f}s | prefill {prompt_eval:.1f}s "
...
def check_fidelity(model: str, wav: str) -> None:
    ...
    text = chat(model,
                ...
                [b64(wav)])
    print(f"\n{text}\n")
    print(">>> VERDICT: compare word-by-word with what you actually said. "
...
def check_intonation(model: str, neutral_wav: str, emotional_wav: str) -> None:
    ...
    print(">>> VERDICT: tone descriptions must differ AND match reality. "
...
def check_ocr(model: str, png: str) -> None:
    text = chat(model,
                ...
                [b64(png)], visual_token_budget=1120)
    print(f"\n{text}\n")
    print(">>> VERDICT: check the smallest real font (IDE, terminal). "
...
if __name__ == "__main__":
    ...
    fn = {"fidelity": check_fidelity, "intonation": check_intonation,
          "ocr": check_ocr, "vram": check_vram}[a.check]
    fn(a.model, *a.files)
